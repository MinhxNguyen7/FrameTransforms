...
if __name__ == "__main__":
    ...
    register.add_transform(Frame.WORLD, Frame.BASE, world_to_base_transform)

    base_to_camera_transform = make_3d_transformation(
        np.array([0, 1, 0]), Rotation.from_euler("xyz", [0, 90, 0], degrees=True)
    )
    register.add_transform(Frame.BASE, Frame.CAMERA, base_to_camera_transform)

    # Attempt to add a transformation that creates a cycle
    try:
        register.add_transform(Frame.CAMERA, Frame.WORLD, np.zeros(4))
    ...
    actual = register.get_transform(Frame.WORLD, Frame.CAMERA)
